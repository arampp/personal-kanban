...
internal class MediatRSenarioBuilder<TRequest> where TRequest : IRequest
{
    ...
    internal async Task<TNotification> AndExpectNotification<TNotification>() where TNotification : INotification
    {
        ...
        var testContext = TestContext.New()
            .ConfigureServices(services => services.AddTransient<INotificationHandler<TNotification>>(s => handler))
            .Build();


        await testContext.Mediator.Send(_request);
    ...
    {
        var testContext = TestContext.New().Build();
